...
export default function DashboardPage() {
  ...
  const calculateTotalBalance = (): number => {
    ...
    return accounts.reduce((total, account) => {
      return total + account.balances.reduce((accountTotal, balance) => {
  ...
  return (
    <MainLayout>
      <div className="space-y-8">
        {/* Header */}
        <div 
          className={`text-center lg:text-left transition-opacity duration-500 ${animationsReady ? 'opacity-100' : 'opacity-0'}`}
          style={{ 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className={`card-elevated p-6 hover:scale-105 transition-all duration-300 ${animationsReady ? 'animate-bounce-in' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.1s' : '0s' }}>
          ...
          <div className={`card-elevated p-6 hover:scale-105 transition-all duration-300 ${animationsReady ? 'animate-bounce-in' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.2s' : '0s' }}>
            <div className="flex items-center">
              <div className="p-3 bg-green-500/20 rounded-xl shadow-lg">
                <TrendingUp className="h-6 w-6 text-green-500" />
              </div>
              <div className="ml-4">
                <p className="text-sm font-medium text-muted-foreground">Income (Last Month)</p>
                <div className="flex items-center space-x-2">
                  {ratesLoading ? (
                    <div className="h-6 w-6 border-2 border-muted-foreground border-t-transparent rounded-full animate-spin"></div>
                  ) : (
          ...
          <div className={`card-elevated p-6 hover:scale-105 transition-all duration-300 ${animationsReady ? 'animate-bounce-in' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.3s' : '0s' }}>
            <div className="flex items-center">
              <div className="p-3 bg-red-500/20 rounded-xl shadow-lg">
                <TrendingDown className="h-6 w-6 text-red-500" />
              </div>
              <div className="ml-4">
                <p className="text-sm font-medium text-muted-foreground">Expenses (Last Month)</p>
                <div className="flex items-center space-x-2">
                  {ratesLoading ? (
                    <div className="h-6 w-6 border-2 border-muted-foreground border-t-transparent rounded-full animate-spin"></div>
                  ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className={`card-elevated p-6 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.5s' : '0s' }}>
            ...
            {accounts.length === 0 ? (
              ...
              <div className="space-y-3">
                {accounts.map((account, index) => (
                  <div key={account.id} className="flex justify-between items-center p-4 bg-muted/30 rounded-xl hover:bg-muted/50 transition-colors duration-200 animate-slide-in" style={{ animationDelay: `${index * 100}ms` }}>
                    <div>
                    ...
                    <div className="text-right">
                      {ratesLoading ? (
                        ...
                        <span className="font-semibold text-foreground">
                          {formatCurrency(calculateAccountTotal(account), mainCurrency)}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className={`card-elevated p-6 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.6s' : '0s' }}>
            <h2 className="text-xl font-semibold text-foreground mb-6 flex items-center">
            ...
            {recentTransactions.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">No recent transactions. Start tracking your expenses and income.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {recentTransactions.map((transaction, index) => (
                  <div key={transaction.id} className="flex justify-between items-center p-4 bg-muted/30 rounded-xl hover:bg-muted/50 transition-colors duration-200 animate-slide-in" style={{ animationDelay: `${index * 100}ms` }}>
                    <div>
                      <p className="font-medium text-foreground">
                        {transaction.description && transaction.description}
                        {!transaction.description && transaction.type === 'income' && 'Income'}
                        {!transaction.description && transaction.type === 'expense' && 'Expense'}
                        {!transaction.description && transaction.type === 'transfer' && 'Transfer'}
                      </p>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <p className={`font-semibold ${
                      transaction.type === 'income' ? 'text-green-500' : transaction.type === 'expense' ? 'text-red-500' : 'text-blue-500'
                    }`}>
                      {transaction.type === 'income' ? '+' : transaction.type === 'expense' ? '-' : ''}{formatCurrency(transaction.amount, transaction.currency)}
                    </p>
                  </div>
                ))}
              </div>
