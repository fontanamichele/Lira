...
import { Plus, Edit, Trash2, TrendingUp, TrendingDown, Calendar, ArrowRightLeft } from 'lucide-react'
    ...
    try {
      ...
      if (editingTransaction) {
        ...
        if (formData.type === 'transfer') {
          ...
          updateData.to_account_balance_id = formData.to_account_balance_id
        ...
        if (formData.type === 'transfer') {
          ...
          insertData.to_account_balance_id = formData.to_account_balance_id
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        ...
        {/* Transactions List */}
        <div className={`bg-card border border-border rounded-lg transition-all duration-300 ${animationsReady ? 'animate-slide-in-up' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.3s' : '0s' }}>
          <div className="p-6 border-b border-border">
            <h2 className="text-lg font-semibold text-foreground">Recent Transactions</h2>
          </div>
          <div className="divide-y divide-border">
            {transactions.map((transaction, index) => (
              <div key={transaction.id} className={`p-6 hover:bg-muted/50 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? `${index * 0.05}s` : '0s' }}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                        {transaction.type === 'transfer' ? (
                          <>
                            <span>{getAccountName(transaction.account_id)} → {getToAccountName(transaction.to_account_id!)}</span>
                            <span>•</span>
                            <span>{formatCurrency(transaction.amount, transaction.currency)} → {formatCurrency(transaction.to_amount!, transaction.to_currency!)}</span>
                          </>
                        ) : (
                          <>
