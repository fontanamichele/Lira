  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        {/* Header */}
        <div 
          className={`transition-opacity duration-500 ${animationsReady ? 'opacity-100' : 'opacity-0'}`}
          style={{ 
        ...
        {/* Profile Settings */}
        <div className={`bg-card border border-border rounded-lg p-6 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.1s' : '0s' }}>
          <div className="flex items-center mb-6">
            <div className="p-2 bg-primary/10 rounded-lg">
              <User className="h-5 w-5 text-primary" />
            </div>
            <h2 className="text-lg font-semibold text-foreground ml-3">Profile Information</h2>
          </div>
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <div>
              <label htmlFor="main_currency" className="block text-sm font-medium text-foreground mb-2">
                Main Currency
              </label>
              <select
                id="main_currency"
        ...
        {/* Security Settings */}
        <div className={`bg-card border border-border rounded-lg p-6 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.2s' : '0s' }}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              <div className="p-2 bg-blue-500/10 rounded-lg">
                <Eye className="h-5 w-5 text-blue-500" />
              </div>
              <h2 className="text-lg font-semibold text-foreground ml-3">Security</h2>
            </div>
            <button
              onClick={() => setShowPasswordForm(!showPasswordForm)}
              className="flex items-center px-3 py-1 text-sm text-muted-foreground hover:text-foreground"
            >
              {showPasswordForm ? <EyeOff className="h-4 w-4 mr-1" /> : <Eye className="h-4 w-4 mr-1" />}
        ...
        {/* Theme Settings */}
        <div className={`bg-card border border-border rounded-lg p-6 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.3s' : '0s' }}>
          <div className="flex items-center mb-6">
            <div className="p-2 bg-purple-500/10 rounded-lg">
              <Palette className="h-5 w-5 text-purple-500" />
            </div>
            <h2 className="text-lg font-semibold text-foreground ml-3">Appearance</h2>
          </div>
        ...
        {/* Account Information */}
        <div className={`bg-card border border-border rounded-lg p-6 transition-all duration-300 ${animationsReady ? 'animate-slide-in-right' : 'opacity-0'}`} style={{ animationDelay: animationsReady ? '0.4s' : '0s' }}>
